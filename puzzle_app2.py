...
def puzzle():
  # fetch the uploaded user image to a temporary file
  img = request.files.get('file')
  if(img == None):
    # user didn't send us an image file
    print("no image file!")
    #abort(400)
  elif(not allowed_file(img.filename)):
    # user didn't give us the right file type
    print("not a png file!")
    #abort(400)
  img_tempfile = tempfile.NamedTemporaryFile()
  img.save(img_tempfile.name)

  # generate our stl
  stl_tempfile = tempfile.NamedTemporaryFile()
  make_puzzle(img_tempfile.name, stl_tempfile.name)

  # send back the puzzle as a download
  return send_file(stl_tempfile.name,
                         as_attachment=True,
                         attachment_filename="puzzle.stl")

def make_puzzle(img_filename, stl_filename):
  HEIGHT_MULTIPLER = 256
  HEIGHT_BASE = 128

  # load the user image as a MxNx4 (rgba) numpy array
  user_img = imread(img_filename, format="png")
...
def make_puzzle_from_img(filename):
    ...
    # load a mask for the puzzle pieces where black zeros the surface height
    mask_img = imread("fitted4x4template-500x500.png")
    mask_img_grey = (0.5*mask_img[:,:,2]) + (0.5*mask_img[:,:,0])
